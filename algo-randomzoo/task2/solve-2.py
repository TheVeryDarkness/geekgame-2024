...
def test_seed_mt():
    ...
    with open('random.txt', 'r') as f:
        numbers = [int(i) for i in f.read().split()][:624]
        # rand_seed = urandbits(32)
        # r = MT19937(rand_seed)
        # outputs = [(i,r.extract_number()) for i in range(5)]
        outputs: list[tuple[int, int | tuple[int, int]]] = [(i, num-ord("flag{"[i])) if i < 5 else (i,
                                                                                                    (num-0x7e, num-0x20)) for i, num in enumerate(numbers)]
        print(outputs)
        b = Breaker()
        ...
        r = MT19937(recovered_seed)
        for i, num in enumerate(numbers):
            print(chr(num - r.extract_number()), end="")
