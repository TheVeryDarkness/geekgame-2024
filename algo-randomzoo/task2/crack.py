...
from z3 import BitVec, Solver, LShR, Extract, If
...
class Untwister:
    ...
    def get_random(self):
        ...
        print(list(map(lambda x: model[x], self.MT)))
        # state = []
        # for x in self.MT:
        #     m = model[x]
        #     if m is not None:
...
def test1():
    ...
    recovered: str = ""
    for j in range(5, len(random_numbers)):
        c = random_numbers[j] - r.getrandbits(32)
        print(c)
        # recovered += chr(c)
    # print(recovered)
